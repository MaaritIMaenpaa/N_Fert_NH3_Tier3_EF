---
title: "2023 NH3-loss Statistical Models Micromet only Outdoor"
...
# Subset to only micromet and outdoor data
NH3predict <- filter(NH3predict, Method=="Micromet" & Place=="Outdoor")
...
ddply(NH3predict, c("Fertiliser", "FertiliserType"), summarise, N=length(NH3loss))
...
table(NH3predict$Fertiliser, NH3predict$Application)
...
model <- lmer(sqrt(NH3loss +4) ~ Fertiliser + Application + (1|Ref),
...
# Model 1 
m1_lmer_p1 <- lmer(sqrt(NH3loss+4) ~ Fertiliser + Application + (1|Ref),
                   #weights=Weight,
                   control=lmerControl(optimizer="bobyqa"),
                   data=NH3predict)
...
plot(fitted(m1_lmer_p1), resid(m1_lmer_p1)) 
abline(h=0) # OK
...
NH3-loss was different for different fertilisers (F(`r round(anova(m1_lmer_p1)$NumDF[1], 0)`, `r round(anova(m1_lmer_p1)$DenDF[1],0)`) =`r round(anova(m1_lmer_p1)$F[1], 2)`, p `r ifelse(round(anova(m1_lmer_p1)[[6]][1], 3)=="0", "<0.001", paste("=",round(anova(m1_lmer_p1)[[6]][1], 3)))`), as were the emissions from different application methods (F(`r round(anova(m1_lmer_p1)$NumDF[2], 0)`, `r round(anova(m1_lmer_p1)$DenDF[2],0)`) =`r round(anova(m1_lmer_p1)$F[2], 2)`, p `r ifelse(round(anova(m1_lmer_p1)[[6]][2], 3)=="0", "<0.001", paste("=",round(anova(m1_lmer_p1)[[6]][2], 3)))`). Overall, the measurement method did explain variation in NH3 emissions (F(`r round(anova(m1_lmer_p1)$NumDF[3], 0)`, `r round(anova(m1_lmer_p1)$DenDF[3],0)`) =`r round(anova(m1_lmer_p1)$F[3], 2)`, p `r ifelse(round(anova(m1_lmer_p1)[[6]][3], 3)=="0", "<0.001", paste("=",round(anova(m1_lmer_p1)[[6]][3], 3)))`).
...
# Vectors for emmeans-objects and vectors
Model_vec <- c("m1_est_F", "m1_est_A")
Factor_vec <- c("Fertiliser","Application")
...
Model 1 explains `r round(r.squaredGLMM(m1_lmer_p1)[2]*100, 2)` % of the variation in the data (conditional R2), with the fixed effects explaining `r round(r.squaredGLMM(m1_lmer_p1)[1]*100, 2)` % (marginal R2). The R2-values for linear mixed effects model were calculated according to Nagakawa & Schielzeth (2013).
...
```{r include=TRUE, echo=FALSE}
hist(NH3predict_m2$Application.rate, breaks=50,xlim = c(0,500))
...
Application rate explained a significant proportion of the variation in NH3-emissions (F(`r anova(m2_lm)$Df[1]`, `r anova(m2_lm)$Df[2]`) =`r round(anova(m2_lm)$F[1], 2)`, p `r ifelse(round(anova(m2_lm)$P[1], 3)=="0", "<0.001", paste("=",round(anova(m2_lm)$P[1], 3)))`). There was a statistically significant positive trend between Application rate and NH3-emissions ($\beta$+/-SE = `r round(coef(m2_lm)[[2]],5)` +/- `r round(summary(m2_lm)$coef[2,2],5)`).

There are `r length(NH3predict_m2$NH3loss)` observations in the analysis.
...
ggplot(NH3predict_m2, aes(y=NH3loss, x=Application.rate)) + 
  ...
  geom_smooth(aes(y=(M2_fits^2)-4), method='lm', se=T, colour="black") + xlim(0,500) + ylim(0,80)+
...
The effect of soil pH on NH3-emissions was dependent on the Fertiliser types (F(`r anova(m3_lm)$Df[3]`, `r anova(m3_lm)$Df[4]`) =`r round(anova(m3_lm)$F[3], 2)`, p `r ifelse(round(anova(m3_lm)$P[3], 3)=="0", "<0.001", paste("=",round(anova(m3_lm)$P[3], 3)))`). There was no independent effect of soil pH on NH3 emissions (F(`r anova(m3_lm)$Df[2]`, `r anova(m3_lm)$Df[4]`) =`r round(anova(m3_lm)$F[2], 2)`, p `r ifelse(round(anova(m3_lm)$P[2], 3)=="0", "<0.001", paste("=",round(anova(m3_lm)$P[2], 3)))`).

There are `r length(NH3predict_m3$NH3loss)` observations in the analysis.
...
m3_lmer <- lmer(sqrt(NH3loss+4) ~ Fertiliser + Application + pH_cat + Fertiliser:pH_cat + (1|Ref),
                   weights=Weight,
                   control=lmerControl(optimizer="bobyqa"),
                   data=NH3predict)
```
...
# Effect sizes
at <- list("Application"="Broadcast", "pH_cat"="normal")
...
# Effect sizes
at <- list("Fertiliser"="urea+", "pH_cat"="normal")
...
Model_vec <- c("m3_est_F", "m3_est_A", "m3_est_pH")
Factor_vec <- c("Fertiliser", "Application", "Soil pH")
...
There are `r length(NH3predict_m4$NH3loss)` observations in the analysis.
...
There are `r length(NH3predict_m5$NH3loss)` observations in the analysis.
...
##Plot effect of continuous variables Clay and Air temperature on a 3D graph
##Only for urea++

z_values<-function(xRain,yTemp){
  ((mTR_lm$coefficients[1]+mTR_lm$coefficients[2]*yTemp)^2-4+ mTR_lm$coefficients[3]*xRain + mTR_lm$coefficients[4]*yTemp*xRain)
}
xRain<- 0:15
yTemp<- -5:31
z=outer(xRain,yTemp,z_values)

ylimRange=range(yTemp)
ylimRange[1]=ylimRange[1]-5
ylimRange[2]=ylimRange[2]+4
zlimRange=range(z)
zlimRange[1]=0
zlimRange[2]=30
persp(xRain,yTemp,z,xlim=range(xRain), ylim=ylimRange, zlim=zlimRange, xlab='Rain (mm)', ylab='Air temp.(Celsius)', zlab='NH3-N (% N applied)',main='Rainand temperature',ticktype='detailed', theta = 30, phi = 15)

# Another figure

library(plotly)

NH3predict_m7$mTR_fit <- as.numeric(as.character(fitted(mTR_lm)))

p <- plot_ly(x=filter(NH3predict_m7, Fertiliser=="urea+")$Rain.mean, 
             y=filter(NH3predict_m7, Fertiliser=="urea+")$AirTemperature,
             z=filter(NH3predict_m7, Fertiliser=="urea+")$mTR_fit,
             type="scatter3d", # mesh3D 
             mode="markers")
        
p |>
  layout(scene=list(xaxis=list(title="Rainfall", nticks = 20), 
                          yaxis=list(title="Air temp.(Celsius)", nticks = 20), 
                          zaxis=list(title="NH3-N (% N applied)", nticks = 20)))
...
ggplot(NH3predict_m7, aes(x=Rain.mean, y=NH3loss))+
  ...
  geom_line(stat = "smooth", 
            ...
            size=1,
            se = FALSE) +
  labs(x="Rainfall (mean mm/day)", y="sqrt(NH3 loss (%))")
...
```{r include=TRUE, echo=FALSE}
hist(as.numeric(as.character(NH3predict_m9$irrigation)), breaks=50)
...
NH3predict_mALL<- NH3predict_mALL %>% drop_na(c(AirTemperature))

```
Total number of observations `r length(NH3predict_mALL$NH3loss)`
```{r include=TRUE, echo=TRUE}
...
Model 11 explains `r round(r.squaredGLMM(mALL_lmer)[2]*100, 2)` % of the variation in the data (conditional R2), with the fixed effects explaining `r round(r.squaredGLMM(mALL_lmer)[1]*100, 2)` % (marginal R2).
...
# Vectors for emmeans-objects and fectors
Model_vec <- c("mALL_est_F", "mALL_est_A", "mALL_est_pH")
Factor_vec <- c("Fertiliser", "Application", "Soil pH")
...
Total observations of`r length(NH3predict_mALL2$NH3loss)`

```{r include=TRUE, echo=FALSE}

# Model assumptions
par(mfrow=c(1,4))
qqnorm(resid(mALL_lmer2))
qqline(resid(mALL_lmer2))
#plot(fitted(mALL_lmer2), NH3predict_mALL2$Weight*resid(mALL_lmer2))
abline(h=0)
hist(resid(mALL_lmer2))
par(mfrow=c(1,1))
...
Model 12 explains `r round(r.squaredGLMM(mALL_lmer2)[2]*100, 2)` % of the variation in the data (conditional R2), with the fixed effects explaining `r round(r.squaredGLMM(mALL_lmer2)[1]*100, 2)` % (marginal R2).
...
mALL_glmm <- glmmTMB(NH3loss ~ Fertiliser + Application + 
                    pH_cat + Fertiliser:pH_cat + 
                    Clay + AirTemperature +
                    (1|Ref),
                   dispformula= ~ Fertiliser + Application + pH_cat,
...
Model 16 explains `r round(r.squaredGLMM(mALL_glmm)[2]*100, 2)` % of the variation in the data (conditional R2), with the fixed effects explaining `r round(r.squaredGLMM(mALL_glmm)[1]*100, 2)` % (marginal R2). The R2-values for linear mixed effects model were calculated according to Nagakawa & Schielzeth (2013).
...
# Vectors for emmeans-objects and fectors
Model_vec <- c("mALLglmm_est_F", "mALLglmm_est_A", "mALLglmm_est_pH")
Factor_vec <- c("Fertiliser", "Application", "Soil pH")
